...
class FlareSystem:
    ...
    def emit_fountain(self, pos: Vec2, *, facing_x: float, heli_vel: Vec2 | None = None) -> None:
        if len(self.particles) >= self.max_particles:
            return
        ...
        for _ in range(count):
            ...
            back_speed = self._rng.uniform(self.back_speed_min, self.back_speed_max)
            vx = behind_sign * back_speed + (-hvx * 0.12) + self._rng.uniform(-35.0, 35.0)
            vy = self._rng.uniform(self.up_speed_min, self.up_speed_max) + (-hvy * 0.06)

            ttl = self._rng.uniform(self.ttl_min_s, self.ttl_max_s)
